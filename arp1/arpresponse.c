...
void send_arpresponse(unsigned char hlen, u_int16_t arp_hardware_type, u_int32_t sip,u_int32_t dip, 
						char *sha, int ifindex,int flag,int count,char *devname,int bcstflag){
	int sockfd;
...
//#endif
	...
	memcpy((char *)(arpheader+1)+hlen+4, haddr.sa_data, hlen);
...
#endif
*/
							send_arpresponse(hlen, ARPHRD_ETHER, sip, dip, shaddr,
											ifindex,flag,count,ifi_name,bcstflag);
			}/*end for*/
	}/*end for*/
}
